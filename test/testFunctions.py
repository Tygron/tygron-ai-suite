...
class TestFunction(unittest.TestCase):
    ...
    def testInvalidDatasetPath(self):
        ...
        config.setDatasetPaths("./assets/nonexisting/train/",
                               "./assets/nonexisting/test/")
    ...
    def testValidDatasets(self):
        ...
        config = createTestConfiguration()
        config.setDatasetPaths("./assets/dataset_a/train/",
                               "./assets/dataset_a/test/")
        trainingSet = ImageDataset(config, isTraining=True)
        testSet = ImageDataset(config, isTraining=False)
    ...
    def testMissingFiles(self):
        ...
        for i in range(0,len(paths)):
            ...
            config.setDatasetPaths("./assets/dataset_invalid/"+paths[i], "")
            trainingSet = ImageDataset(config, isTraining=True)
    ...
    def testAutolimitLabelInDataset(self):
        ...
        config = createTestConfiguration()
        config.setDatasetPaths("./assets/dataset_label/train/",
                               "./assets/dataset_label/test/")
        trainingSet = ImageDataset(config, isTraining=True)
        testSet = ImageDataset(config, isTraining=False)
